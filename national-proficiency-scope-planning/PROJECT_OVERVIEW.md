...
**CORRECTED (subscription model):** because the app is subscription-based, an Approved Instructor does **not** automatically get full regular-user access just by being an instructor. By default they get only the **instructor module** — running and approving student qualifications. Full access to everything else (messaging, their own profile/tier progression, media, home, leaderboard, etc.) requires the instructor to **also pay for a subscription** (Free/Mid/Premium), exactly like any other user. This reverses the earlier stated direction for S-008, and the outdated decision entry in `DECISIONS.md` has been marked superseded rather than silently edited.

**Instructor vetting questionnaire (new detail on S-015):** the vetting process now includes concrete questionnaire fields — which organization the applicant is an approved instructor through (e.g. NRA, USCCA, or similar), how many classes they teach per month on average, their company website, and similar questions to confirm they run a real, active organization suitable to partner with National Proficiency. The client gave these as examples, not an exhaustive list — see `Q-023`.
...
**This is flagged as the single highest-priority risk logged for this project so far (R-006).** National Proficiency's core pillars include Firearms and Medical training. A children's program with *reduced* vetting, in that context, runs into serious legal and child-safety territory: children's-data-privacy law (e.g. COPPA), laws governing minors and firearms (which typically require *more* supervision, not less), platform liability for connecting minors with instructors even when "left up to parents," and background-check standards for adults working with children (often *stricter* than general adult vetting, not lighter). **Recommendation: get legal counsel review before any design or build work touches this area** — see `OPEN_QUESTIONS.md` (Q-036–Q-038) for the specific questions that review should answer.
...
**Top priority: legal counsel review of the minor/children's-program concept (R-006)** before any design or build work touches it. Also receive the client's qualifications/training standards document (Q-002, Q-003, S-011) once available, resolve the ID-verification vendor scope (Q-025, R-005), and nail down the corrected instructor-access boundary (Q-045–Q-047) since it affects the paywall/entitlement logic for every instructor account. In parallel, resolve P0 items: account/auth rules including the serial number vs. login question (Q-005, Q-022), Mid-tier feature list and tier pricing (Q-009, Q-010), the raffle legal/compliance risk (R-001), instructor vetting criteria and downgrade-approval authority (Q-015, Q-021, Q-023), whether instructor registration/vetting is web or in-app (Q-019), and legal review/drafting of both waivers (S-016).
